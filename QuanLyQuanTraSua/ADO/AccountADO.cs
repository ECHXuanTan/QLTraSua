...
namespace QuanLyQuanTraSua.ADO
{
    public class AccountADO
    {
        ...
        public static AccountADO Instance
        {
            ...
            {
                if (instance == null)
                    instance = new AccountADO();
                return AccountADO.instance;
            }
            set { AccountADO.instance = value; }
        }
        private AccountADO() { }
        ...
        public bool UpdatePerAccount(string userName, string displayName, string pass, string newPass)
        {
        ...
        {
            string query = string.Format("update Account set Pass = N'0' where UserName = N'{0}'", name);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
